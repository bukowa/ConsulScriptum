...
This will generate the necessary debugger files and automatically open the interface in your default web browser. 

::: warning Safe Path
To bypass browser security restrictions (which block access to `Program Files`), the debugger now saves all state and command files to a "neutral" system location:
`C:\Users\Public\consul\<GAME>\uidebug`
:::

## Connecting the Debugger

Because the debugger runs as a web page, it requires permission to read the state files generated by the game.

1. **Copy the Path**: In the browser window, look for **STEP 1**. Click the dashed box to copy the expected debug path to your clipboard.
2. **Click Connect**: Click the **Connect to Folder** button (**STEP 2**).
3. **Paste & Select**: In the folder picker that appears, paste the path into the address bar at the top and press Enter. Then click **Select Folder**.
4. **Grant Access**: Click **Select Folder** in the browser's security popup.
